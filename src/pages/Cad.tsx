...
import { ArrowLeft, X, ChevronLeft, ChevronRight, Monitor } from "lucide-react";
...
const CadProjects: React.FC = () => {
  ...
  // All CAD images for the unified gallery
  const allCadImages = cadDrawings.map((image, index) => ({
    src: image.src,
    alt: image.alt,
    title: `CAD Project ${index + 1}`,
    category: getCadCategory(index),
    software: getCadSoftware(index),
  }));

  function getCadCategory(index: number) {
    const categories = [
      "3D Modeling",
    ...
    return categories[index] || "CAD Design";
  }

  function getCadSoftware(index: number) {
    const software = [
      "AutoCAD",
      "Revit",
      "SketchUp",
    ...
    return software[index] || "CAD Software";
  }

  const openImageModal = (imageIndex: number) => {
    setCurrentImageIndex(imageIndex);
    setSelectedImage(allCadImages[imageIndex].src);
  };
  ...
  const nextImage = () => {
    if (currentImageIndex < allCadImages.length - 1) {
      const newIndex = currentImageIndex + 1;
      setCurrentImageIndex(newIndex);
      setSelectedImage(allCadImages[newIndex].src);
    }
  };
          ...
          <div className="text-center mb-16">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              ...
              <span className="block text-gray-900">
                Computer-Aided Design Gallery
              </span>
            </h1>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
              Browse through my comprehensive digital design portfolio including
              3D modeling, architectural visualization, and computer-aided
              design projects using industry-standard software. Click any image
              to view in detail.
            </p>
            ...
            <div className="flex items-center justify-center space-x-6 mt-8">
              <div className="flex items-center space-x-2 text-gray-600">
                ...
                <span className="font-medium">
                  {allCadImages.length} CAD Projects
                </span>
              </div>
            </div>
          </div>
              ...
              >
                <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500">
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-gray-400 text-xs">{image.software}</p>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        >
          <div className="relative max-w-7xl max-h-full">
            ...
            <button
              ...
              disabled={currentImageIndex === allCadImages.length - 1}
            ...
            <div className="flex flex-col items-center justify-center max-w-full max-h-full">
              <img
                src={selectedImage}
                alt={allCadImages[currentImageIndex].alt}
              ...
              {/* <div className="mt-4 text-center">
                <h3 className="text-white text-lg font-semibold mb-1">
                  {allCadImages[currentImageIndex].title}
                </h3>
                <p className="text-gray-300 text-sm mb-1">
                  {allCadImages[currentImageIndex].category}
                </p>
                <p className="text-gray-400 text-xs">
                  Created with {allCadImages[currentImageIndex].software}
                </p>
              </div> */}
            </div>
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center space-x-4">
              <div className="bg-white/10 text-white px-4 py-2 rounded-full text-sm">
                {currentImageIndex + 1} / {allCadImages.length}
              </div>
            </div>
          </div>
        </motion.div>
